...
[`docs/02-architecture.md`](../02-architecture.md) / [`.claude/rules/server-patterns.md`](../../.claude/rules/server-patterns.md) を参照。
...
4. `関連ファイル / 機能概要 / シナリオ / 失敗 / 例外` を記述。
   概念カードは `シナリオ` の代わりに `設計意図 / 主要メンバー / 関連する動作` を使う
...
7. `specre index` を実行（pre-commit hook で自動化するのが望ましい）
