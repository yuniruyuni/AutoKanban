...
**CodingAgentProcess は、1 回 Agent サブプロセスを spawn した実行ライフサイクルを表す
...
`Session.executor` に保存される Agent id（例: `claude-code`, `gemini-cli`, `codex-cli`）によって、
`ExecutorRepository` が対応する driver を選ぶ。Agent の意味定義は
[Agent](../agent/agent_is_the_coding_agent_definition.md) に置き、subprocess の実装詳細は
repository / infra 側に閉じる。
...
多くの CLI 型 agent では、1 プロセスは 1 回の executor invocation として扱う。
Claude Code は「起動時に渡した `--resume <sessionId>` で始まる会話」を 1 プロセスが保持し、
Codex CLI も `codex exec resume <session>` で新しい non-interactive process を起動して継続する。
既に終了したプロセスを再利用することはできず、実行中プロセスに任意の会話を後から差し替えることも
agent ごとに制約が異なる。
...
CodingAgentProcess は DB エンティティなので外部 process 実装を知らない。
起動時の依存は次のように分かれる。

```mermaid
flowchart LR
  Session["Session.executor<br/>agent id"]
  Repo["ExecutorRepository<br/>agent id -> driver"]
  Driver["repositories/executor/drivers/*<br/>ICodingAgentDriver"]
  Infra["infra/executor/*<br/>CLI subprocess adapter"]
  Process["外部 agent process"]

  Session --> Repo --> Driver --> Infra --> Process
```

`repositories/executor/drivers/codex-cli` は AutoKanban の callback / log / process lifecycle
に合わせる adapter であり、`infra/executor/codex-cli` は `codex exec --json` を spawn するだけの
低レベル adapter である。infra は `CodingAgentProcess` や `CodingAgent` を import しない。
...
Process 自体には executor id を持たせず、`sessionId -> Session.executor` で辿る。
これにより同じ Session 内の process 群は同じ Agent 文脈で扱われ、follow-up / structured log parser
も Session を起点に選べる。
