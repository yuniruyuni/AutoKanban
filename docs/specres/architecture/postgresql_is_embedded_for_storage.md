...
```mermaid
flowchart TB
    S1["EmbeddedPostgresManager.start()"]
    S1a["初回: PG バイナリ DL + initdb"]
    S1b["PG プロセス起動"]
    S2["ensurePgSchema(connectionParams)<br/>schema.sql の差分を pgschema が適用"]
    S3["new PgDatabase(poolConfig)<br/>pg.Pool で接続"]
    S4["recoverOrphanedProcesses(db)<br/>前回起動の orphan process を cleanup"]
    S5["seedDefaults(db)<br/>デフォルト variant / template 投入"]
