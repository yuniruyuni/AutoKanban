...
- **AK_\* 環境変数**: prepare / cleanup スクリプトにも `AK_PROCESS_ID` / `AK_SESSION_ID` /
  `AK_TASK_ID` / `AK_WORKSPACE_ID` / `AK_PROJECT_ID` / `AK_WORKTREE_PATH` が注入される。
  これによりプロジェクト側のスクリプトが「この実行は何 task / 何 workspace の下か」を
  自分で判別できる（例: 本リポジトリの `cleanup` が `AK_WORKSPACE_ID` で isolated state を掃除する）
  — 詳細は [ak_env_context_is_exported_to_spawned_scripts](../architecture/ak_env_context_is_exported_to_spawned_scripts.md)
