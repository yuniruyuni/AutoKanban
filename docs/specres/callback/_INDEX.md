# callback

| Name | Status | Last Verified |
|------|--------|---------------|
| [process_completion_updates_task_status](process_completion_updates_task_status.md) | draft | - |
| [process_idle_triggers_next_queued_message](process_idle_triggers_next_queued_message.md) | draft | - |
| [session_info_is_recorded_for_resume](session_info_is_recorded_for_resume.md) | draft | - |
| [log_data_is_persisted_and_broadcast](log_data_is_persisted_and_broadcast.md) | draft | - |
| [summary_is_recorded_on_turn_end](summary_is_recorded_on_turn_end.md) | draft | - |
| [approval_request_is_detected_and_persisted](approval_request_is_detected_and_persisted.md) | draft | - |
