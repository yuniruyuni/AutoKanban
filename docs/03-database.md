...
ORMを使用せず、bun:sqliteを直接使用してRaw SQLを記述する方針を採用。
...
export class TaskRepository {
  ...
  private query<R>(fragment: SQL): R[] {
    return this.db.query(fragment.sql).all(...fragment.params) as R[];
  }

  private queryOne<R>(fragment: SQL): R | null {
    return this.db.query(fragment.sql).get(...fragment.params) as R | null;
  }

  private execute(fragment: SQL): void {
    this.db.query(fragment.sql).run(...fragment.params);
  }
 ...
 * トランザクション内で処理を実行
 */
export function transaction<T>(db: Database, fn: TransactionFn<T>): T {
  db.query('BEGIN').run();
  try {
    const result = fn(db);
    db.query('COMMIT').run();
...
export class GetProjectSummaryUsecase {
  ...
  execute(projectId: string): ProjectSummary | null {
    ...
    `;
    const project = this.db.query(projectFragment.sql)
      .get(...projectFragment.params) as Record<string, unknown> | null;
    ...
    const countFragment = sql`
      ...
      GROUP BY status
    `;
    const counts = this.db.query(countFragment.sql)
      .all(...countFragment.params) as { status: string; count: number }[];
    ...
    const recentFragment = sql`
      ...
      LIMIT 5
    `;
    const recentTasks = this.db.query(recentFragment.sql)
      .all(...recentFragment.params) as Record<string, unknown>[];
