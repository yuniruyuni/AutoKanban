...
`pg` (node-postgres) を `PgDatabase` クラスでラップ。`Database` interfaceを実装:

```typescript
// repositories/common/database.ts - Database interface
interface Database {
...
// repositories/common/pg-client.ts - PostgreSQL実装
class PgDatabase implements Database { ... }
```

### DbCtxパターン

Repositoryメソッドは第一引数に`DbReadCtx`/`DbWriteCtx`/`ServiceCtx`を受け取る。これによりRead/Write/Service操作の分離が型レベルで保証される:

```typescript
// repositories/common/capability.ts
type DbReadCtx = { db: Database };
type DbWriteCtx = { db: Database };
type ServiceCtx = { /* external service access */ };
```
